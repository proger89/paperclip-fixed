...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("instance.heartbeats.updateFailed"));
    },
  });
  ...
  if (heartbeatsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("instance.heartbeats.loading")}</div>;
  }
    ...
    return (
      <div className="text-sm text-destructive">
        {heartbeatsQuery.error instanceof Error
          ...
          : t("instance.heartbeats.loadFailed")}
      </div>
    );
  }
    ...
    <div className="max-w-5xl space-y-6">
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("instance.heartbeats.title")}</h1>
        </div>
        <p className="text-sm text-muted-foreground">{t("instance.heartbeats.description")}</p>
      </div>

      <div className="flex gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {translateText("active")}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {translateText("disabled")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {translateText(grouped.length === 1 ? "company" : "companies")}</span>
      </div>
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("instance.heartbeats.empty")}
        />
      ) : (
                  ...
                  {group.agents.map((agent) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          {translateText(agent.schedulerActive ? "On" : "Off")}
                        </Badge>
                        <Link
                        ...
                        >
                          {agent.lastHeartbeatAt
                            ...
                            : t("instance.heartbeats.lastSeenNever")}
                        </span>
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            to={buildAgentHref(agent)}
                            className="text-muted-foreground hover:text-foreground"
                            title={t("instance.heartbeats.fullConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("instance.heartbeats.disable") : t("instance.heartbeats.enable")}
                          </Button>
                        </span>
                      </div>
                    );
